...
X = [ones(m,1) X];
layerone = sigmoid(X * Theta1');
layerone = [ones(size(layerone,1) , 1) layerone];
output = sigmoid(layerone * Theta2');
...
J = J + sum((sum(-b .* log(output)))' - (sum((1-b) .* log(1 - output)))');
J = J/m;
